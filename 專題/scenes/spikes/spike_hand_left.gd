...
func _ready() -> void:
	...
	# ***修改点 1：RayCast2D 目标位置设置为 X 轴的负方向（向左）***
	player_detector.target_position.x = -attack_distance
	player_detector.target_position.y = 0.0 # 确保 Y 轴检测距离为零
...
func attack():
	...
	# ***修改点 2：攻击目标位置设置为 X 轴的负方向（向左）***
	move_to_position(original_position + Vector2(-attack_distance, 0))
